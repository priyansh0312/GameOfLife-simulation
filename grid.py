...
class Grid:
	...
	# getting the number of adjacent neighbours of a particular cell 
	def getNeighbours(self,i,j):
		...
		count-= self.arr[i][j] # excluding the current cell 
		return count


	# making the next generation of the array : implementing game rules to the cells 
	def next(self):
		...
		for i in range(self.rows):
			for j in range(self.cols):
				...
				# implementing the rules to the cell and assiging values to the current cell in another 2D array.
	...
	# assigning colors to the cell state
